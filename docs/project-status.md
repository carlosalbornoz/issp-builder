...
| Part I-C transaction direction | Done, 2026-07-18 | Every stakeholder transaction/service now carries a required `direction: "INCOMING" \| "OUTGOING" \| ""` (`src/lib/store/types.ts`), matching the DICT 2026 v2 template's Transaction Processed column. Schema bumped to v10; existing files default every service to `""` and flag `part1/c` for migration review. Editor Table/Cards/Summary views (and the shared drawer) all expose a segmented Incoming/Outgoing toggle; the PDF groups each stakeholder's services under INCOMING:/OUTGOING:/UNSPECIFIED: sub-rows. Also flattened a 3-deep nested-card violation in the Cards view and its drawer (per-service boxes → one bordered `divide-y` list) found while auditing these views for this change. |
